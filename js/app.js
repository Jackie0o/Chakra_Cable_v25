        ...
        function initSliders() {
            ...
            productsContainers.forEach((container => {
                ...
                if (slider && prevButton && nextButton) {
                    const isBasketPopupSlider = container.closest("#basket-popup") !== null;
                    new swiper_core_Swiper(slider, {
                        ...
                        breakpoints: {
                            ...
                            992: {
                                speed: 800,
                                slidesPerView: isBasketPopupSlider ? 1.5 : 4,
                                spaceBetween: 15,
                                freeMode: true
                            },
                            1298.98: {
                                speed: 800,
                                slidesPerView: isBasketPopupSlider ? 2.5 : 5,
                                spaceBetween: 20
                            }
                        }
                    });
        ...
        function initCartButtonHandler() {
            ...
            document.querySelectorAll(".product__button").forEach((button => {
                button.addEventListener("click", (() => {
                    const cartBody = document.querySelector(".basket-popup__body");
                    if (cartBody) {
                        cartBody.classList.remove("empty");
                        cartItemCount++;
                        updateCartItemCount();
                    } else console.error('Element with class "basket-popup__body" not found.');
                }));
            }));
            function updateCartItemCount() {
                if (cartItemCountElement) {
                    cartItemCountElement.textContent = cartItemCount > 0 ? cartItemCount.toString() : "";
                    if (cartItemCount > 0) cartItemCountElement.classList.add("active"); else cartItemCountElement.classList.remove("active");
                }
            }
        }
        function initCartItemDeleteHandler() {
            const cartItemCountElement = document.querySelector(".header-actions__basket-count");
            document.querySelectorAll(".basket-popup__delete-button").forEach((button => {
                button.addEventListener("click", (() => {
                    const cartBody = document.querySelector(".basket-popup__body");
                    if (cartBody) {
                        cartBody.classList.add("empty");
                        cartItemCountElement.textContent = "";
                        cartItemCountElement.classList.remove("active");
                    } else console.error('Element with class "basket-popup__body" not found.');
                }));
            }));
        }
